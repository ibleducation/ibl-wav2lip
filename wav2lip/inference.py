...
def face_detect(args, images):
    ...
    for rect, image in zip(predictions, images):
        if rect is None:
            cv2.imwrite(
                ".wav2lip_temp/faulty_frame.jpg", image
...
def main(args):
    BASE_DIR = Path.cwd()
    if not (BASE_DIR / ".wav2lip_temp").is_dir():
        (BASE_DIR / ".wav2lip_temp").mkdir()
    ...
    command = "ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}".format(
        args.audio, ".wav2lip_temp/result.avi", args.outfile
    )
    subprocess.call(command, shell=platform.system() != "Windows")
    if (BASE_DIR / ".wav2lip_temp").is_dir():
        try:
            (BASE_DIR / ".wav2lip_temp").unlink()
        except:
            import shutil

            shutil.rmtree(BASE_DIR / ".wav2lip_temp")
